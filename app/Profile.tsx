// Profile.native.tsx — wersja natywna iOS + Android (mobile-first)
// ============================================================
...
import { db } from "../src/firebase/firebase"; // ✅ native: bez .web
import {
  doc,
  onSnapshot,
  updateDoc,
...
function isMissionDoneByUserOnDate(m: any, date: Date, userId: string | null) {
  ...
  if (repeatType !== "none") {
    ...
    // fallback: jeśli brak wpisu w completedByByDate -> traktujemy assignedTo jako wykonawcę
...
export default function ProfileScreen() {
  ...
  const viewedUid =
    (Array.isArray(rawUid) ? rawUid[0] : rawUid)
      ? String(Array.isArray(rawUid) ? rawUid[0] : rawUid)
      : myUid || null;
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.log("[Profile.native] users snapshot error:", err);
        setPrivateUserDoc(null);
        setPrivateLoading(false);
      }
    );
  ...
  /* -------- OTHER: /public_users/{uid} + fallback /users/{uid} -------- */
  useEffect(() => {
    ...
    const unsubPublic = onSnapshot(
      ...
      (err) => {
        console.log("[Profile.native] public_users snapshot error:", err);
        setPublicUserDoc(null);
        setPublicLoading(false);
      }
    );

    // users fallback (może permission-denied i to OK)
    setPrivateLoading(true);
    ...
    const unsubUsers = onSnapshot(
      ...
      (err) => {
        console.log("[Profile.native] users snapshot (fallback) error:", err);
        setPrivateUserDoc(null);
        setPrivateLoading(false);
      }
    );
  ...
  const userLoading = isOwnProfile
    ? privateLoading
    : publicLoading || (privateLoading && !publicUserDoc);

  // --- Tryb widoczności profilu ---
  const rawMode: PublicProfileMode | null =
    (userDoc?.publicProfileMode as PublicProfileMode | undefined) ||
    (userDoc?.showStats === false ? "basic" : "full");
  ...
  useEffect(() => {
    ...
    setDoc(doc(db, "public_users", myUid), payload, { merge: true }).catch((e) =>
      console.log("[Profile.native] public_users sync error:", e)
    );
  ...
  const handleSetProfileMode = async (mode: PublicProfileMode) => {
    ...
    try {
      ...
      console.log("[Profile.native] set publicProfileMode error:", err);
      setProfileModeOverride(null);
    } finally {
  ...
  return (
    ...
    >
              ...
              <>
                ...
                <View style={{ flexDirection: "row", marginBottom: 14 }}>
                    ...
                    {isPremium ? (
                      ...
                      >
                        ...
                        >
                          ...
                          <Text style={{ color: premiumGold, fontSize: 8, fontWeight: "900", letterSpacing: 0.6 }}>
                            PREMIUM
                          </Text>
                        </View>
                      </View>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ color: colors.textMuted, fontSize: 12, marginTop: 6 }}>
                      Od{" "}
                      <Text style={{ color: colors.text, fontWeight: "700" }}>{memberSinceLabel}</Text>
                    </Text>

                    {canShowMissionStatsToViewer ? (
                      <Text style={{ color: colors.textMuted, fontSize: 12, marginTop: 2 }}>
                        Ukończone misje:{" "}
                        <Text style={{ color: colors.text, fontWeight: "800" }}>{completedMissions}</Text>
                      </Text>
                    ) : (
                      <Text style={{ color: colors.textMuted, fontSize: 12, marginTop: 2 }}>
                        Statystyki misji:{" "}
                        <Text style={{ color: colors.text, fontWeight: "800" }}>ukryte</Text>
                      </Text>
                    )}
                  </View>
                ...
                <View style={{ marginBottom: 10 }}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
                    ...
                    <Text style={{ color: colors.textMuted, fontSize: 12 }}>
                      Do LVL {level + 1}:{" "}
                      <Text style={{ color: colors.text, fontWeight: "800" }}>{toNext} EXP</Text>
                    </Text>
                  </View>
      ...
      {!!userDoc && canShowMissionStatsToViewer && (
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            {/* ✅ segmentowy przełącznik jak WEB */}
            {isOwnProfile && (
              <View
                style={{
                  flexDirection: "row",
      ...
      {!!userDoc && viewingSomeoneElse && !canShowMissionStatsToViewer && (
        ...
        >
          <Text style={{ color: colors.textMuted, fontSize: 12 }}>
            Ten użytkownik ukrywa statystyki misji.
          </Text>
        </View>
      )}
    </ScrollView>
  ...
  return (
    ...
    >
      ...
      <Text style={{ color: colors.text, fontSize: 20, fontWeight: "900" }}>
        {value ?? 0}{" "}
        {suffix && isNumber ? (
          <Text style={{ color: colors.textMuted, fontSize: 13, fontWeight: "700" }}>{suffix}</Text>
        ) : null}
      </Text>
    </View>
  );
