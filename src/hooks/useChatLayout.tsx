 ...
 * - zmienna szerokość szuflady z rodziną
 * - różne maxWidth bąbelków wiadomości
 * - stała wysokość top bara (żeby KeyboardAvoidingView znał offset)
 */
function useChatLayout() {
  const { width, height } = useWindowDimensions();

  const headerHeight = 56; // wysokość top baru + cienka linia
  const isVerySmallWidth = width < 360;
  const isShortScreen = height < 700;

  const drawerWidth = Math.min(260, Math.max(200, width * 0.7));
...
export default function MessagesMobile() {
  ...
  const { drawerWidth, messageMaxWidth, headerHeight, keyboardOffset } =
    useChatLayout();
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      {/* CHAT + INPUT w KeyboardAvoidingView */}
      <KeyboardAvoidingView
        style={{ flex: 1 }}
