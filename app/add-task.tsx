...
export default function AddTaskScreen() {
  ...
  // ✅ family + members (dla familyId i chipsów)
  const { family, members, loading: famLoading } = useFamily() as any;
  const { missions } = useMissions() as any;
  ...
  const familyId = useMemo(() => {
    const fid =
      (family as any)?.id ?? (family as any)?.familyId ?? (family as any)?.fid ?? null;
  ...
  const handleSave = async () => {
    ...
    // ✅ weryfikacja zdjęciem tylko gdy to zadanie "dla kogoś" w rodzinie
    const needsVerification =
      !!familyId && !assignee.isSelf && assignedToUserId !== myUid;
    ...
    try {
      ...
      await createMission({
        ...
        // ✅ creator/assigner/assignee – spójne z dokumentami
        createdByUserId,
        createdByName,
